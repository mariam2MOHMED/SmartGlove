...
//
//     try {
//       if (sectionModel != null) { // Skip loading for "Clinic"
//         await Provider.of<SectionContentProvider>(context, listen: false)
//             .getAlSecsContent(userId, sectionModel!);
//       }
//     } catch (e) {
...
class _SectionContentScreenState extends State<SectionContentScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Get dynamic contents from Firebase
    final dynamicContents = sectionContentProvider.getSectionContents(sectionModel?.id ?? "");

    // Combine static and dynamic contents
    final allContents = [
      ...?sectionModel?.staticContents, // Static items
      ...dynamicContents, // Dynamic items
    ];

    return Scaffold(
      appBar: AppBar(
        title: Text(sectionModel?.name ?? "Section Content"),
        backgroundColor:settingProvider.themeMode==ThemeMode.dark? AppTheme.darksecondary:AppTheme.primary,
      ),
      body: isLoading
          ? _buildLoadingIndicator()
          : allContents.isEmpty
          ? _buildNoTasks()
          : _buildContentList(allContents),
      floatingActionButton: FloatingActionButton(
        backgroundColor: settingProvider.themeMode==ThemeMode.dark? AppTheme.darkprimary:AppTheme.primary,
        onPressed: () async {
          await showDialog(
            context: context,
            builder: (context) => ShowDialogContent(
...
class _buildNoTasks extends StatelessWidget {
